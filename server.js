...
const XML_SCHEMA = 'products-green-lit.batch-metadata.xml';
const XSL_SCHEMA = 'products-green-lit.batch-metadata.xsl';
...
function buildSubmissionRecord({ itemCode, timestamp, submittedBy, notes, originalProduct, updatedProduct, includeTransforms = true }) {
  ...
  return includeTransforms ? addTransformDocuments(submissionRecord) : submissionRecord;
}
...
function fieldsToXml(fields, indent) {
  return Object.entries(fields)
    .map(([name, value]) => `${indent}<field name="${escapeXml(name)}">${escapeXml(value)}</field>`)
    .join('\n');
}

function recordToXml(record, indent = '  ') {
  const metadata = record.jsonFiles?.metadata || {};
  const parentChild = record.jsonFiles?.parentChild || {};
  return `${indent}<record itemCode="${escapeXml(record.itemCode)}">
${indent}  <metadata schema="${escapeXml(metadata.$schema || METADATA_SCHEMA)}">
${indent}    <original>
${fieldsToXml(metadata.originalMetadata || {}, `${indent}      `)}
${indent}    </original>
${indent}    <updated>
${fieldsToXml(metadata.updatedMetadata || {}, `${indent}      `)}
${indent}    </updated>
${indent}  </metadata>
${indent}  <parentChild schema="${escapeXml(parentChild.$schema || PARENT_CHILD_SCHEMA)}">
${fieldsToXml(parentChild.hierarchy || {}, `${indent}    `)}
${indent}  </parentChild>
${indent}</record>`;
}

function buildBatchXml(record) {
  const records = record.records || [record];
  return `<?xml version="1.0" encoding="UTF-8"?>
<?xml-stylesheet type="text/xsl" href="${XSL_SCHEMA}"?>
<productMetadataBatch schema="${XML_SCHEMA}" submittedAt="${escapeXml(record.submittedAt)}" submittedBy="${escapeXml(record.submittedBy)}">
  <notes>${escapeXml(record.notes)}</notes>
${records.map((item) => recordToXml(item)).join('\n')}
</productMetadataBatch>
`;
}

function buildBatchXsl() {
  return `<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:output method="html" encoding="UTF-8" indent="yes"/>
  <xsl:template match="/productMetadataBatch">
    <html>
      <head>
        <title>Product metadata batch</title>
        <style>
          body { background:#050b15; color:#f8fbff; font-family:Arial,sans-serif; padding:2rem; }
          table { border-collapse:collapse; width:100%; }
          th, td { border:1px solid #2d405f; padding:.65rem; text-align:left; vertical-align:top; }
          th { background:#132238; }
          h1, h2 { color:#00a3ff; }
        </style>
      </head>
      <body>
        <h1>Product metadata batch</h1>
        <p><strong>Submitted at:</strong> <xsl:value-of select="@submittedAt"/></p>
        <p><strong>Submitted by:</strong> <xsl:value-of select="@submittedBy"/></p>
        <p><strong>Notes:</strong> <xsl:value-of select="notes"/></p>
        <xsl:for-each select="record">
          <h2>Itemcode <xsl:value-of select="@itemCode"/></h2>
          <table>
            <tr><th>Section</th><th>Field</th><th>Value</th></tr>
            <xsl:for-each select="metadata/updated/field">
              <tr><td>Updated metadata</td><td><xsl:value-of select="@name"/></td><td><xsl:value-of select="."/></td></tr>
            </xsl:for-each>
            <xsl:for-each select="parentChild/field">
              <tr><td>Parent/child</td><td><xsl:value-of select="@name"/></td><td><xsl:value-of select="."/></td></tr>
            </xsl:for-each>
          </table>
        </xsl:for-each>
      </body>
...
function addTransformDocuments(record) {
  return {
    ...record,
    schemas: {
      ...record.schemas,
    ...
    xmlFile: buildBatchXml(record),
    xslFile: buildBatchXsl(),
  };
}
...
async function submitBatchUpdate(req, res) {
  ...
  const batchRecord = addTransformDocuments({
    submittedAt: timestamp,
    submittedBy,
    notes,
