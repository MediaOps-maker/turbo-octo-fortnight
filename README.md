...
- Downloads JSON, XML, and XSL files in the browser and stores a matching JSON submission record on the server. The JSON receipt includes `products-green-lit.metadata.schema` and `products-green-lit.parent-child.schema` documents under `jsonFiles`; the XML/XSL pair is generated from the same schema-shaped receipt data for review in XML-aware workflows.
...
## XML and XSL downloads

Final export/submission also downloads:

- `product-metadata-batch-<timestamp>.xml`, which contains every completed itemcode, editable metadata values, and related parent/child fields based on the JSON schema output.
- `product-metadata-batch-<timestamp>.xsl`, which can render the XML as a readable HTML summary table.
